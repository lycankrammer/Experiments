...
/** Array that holds names and descriptions of the
 * standard C signals.
 * 
 * Conditionally, we also add some commonly used
 * signals.
 */
              ...
              const char *string)
{
	const char *doc = (sig < sh_known && sh_pairs[sig].name)
	                  ? sh_pairs[sig].desc : "unknown signal number";

	if (errno) {
		const char *err = strerror(errno);
		errno = 0;
		if (!sig)
			fprintf(io, SH_HEAD "\t%s:\t%s\n", filename, line, string, err);
